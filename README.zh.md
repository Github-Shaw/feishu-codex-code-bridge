...
- Node.js **≥ 20**
- `codex` CLI 已安装并登录：`npm i -g @openai/codex`，然后运行 `codex` 完成登录。详见 [OpenAI Codex CLI 文档](https://developers.openai.com/codex/cli)。
- 一个飞书 / Lark PersonalAgent 应用（首次启动的扫码向导能帮你创建）

> 默认适配器用 `codex --ask-for-approval never --sandbox danger-full-access exec --json`，效果接近本地无人值守的 coding agent。请优先配好访问控制，不要把 bot 暴露给不可信群聊。
...
4. 成功后凭据写入 `~/.feishu-codex-code-bridge/config.json`
...
启用以后再次 `feishu-codex-code-bridge start`，看到 `✓ 已连接` 就可以在飞书里找 bot 对话了。
...
```
feishu-codex-code-bridge start [-c <config>]   启动 bot
feishu-codex-code-bridge ps                    列出本机所有正在跑的 start 进程
feishu-codex-code-bridge stop <id|#>           终止指定 start 进程（SIGTERM，2s 后 SIGKILL）
feishu-codex-code-bridge --help                列所有命令
```

> 多开同一个 app 时，开放平台会把事件随机推到其中一个长连接。`start` 启动前会检测同 app 已有的进程，TTY 下会询问是否先关掉旧进程；非 TTY 会自动取消。
...
| `/doctor [描述]` | 把最近运行日志和你的描述交给 Codex，自助诊断卡住 / 异常的原因 |
| `/help` | 帮助卡片 |
| 其它 `/xxx` | 原样交给 Codex |
...
| 路径 | 内容 |
|---|---|
| `~/.feishu-codex-code-bridge/config.json` | 应用凭据（App ID / Secret），权限 600 |
| `~/.feishu-codex-code-bridge/sessions.json` | 每个 chat / 话题 的 Codex session id + cwd（+ 可选的 `/timeout` 覆盖） |
| `~/.feishu-codex-code-bridge/workspaces.json` | 工作空间映射 |
| `~/.feishu-codex-code-bridge/processes.json` | 当前在跑的 start 进程注册中心（`ps`/`stop` 用），死进程会被自动清理 |
| `~/.feishu-codex-code-bridge/media/<chatId>/` | 下载的图片 / 文件，24h 自动清理 |
| `~/.feishu-codex-code-bridge/logs/YYYY-MM-DD.log` | 结构化运行日志（JSON line），按天滚动；启动时清理超过 7 天的老文件（`FEISHU_CODEX_BRIDGE_LOG_DAYS` 环境变量可改）；`/doctor` 命令读它做诊断 |

> 这个 Codex 版本默认使用独立目录，不会复用原 Claude 版的 `~/.lark-channel` 数据。
...
不太想登飞书也可以，`/config` 表单背后写的是 `~/.feishu-codex-code-bridge/config.json` 的 `preferences.access`：
...
**图片发过去 Codex 说看不到**：bridge 会把图片下载到本地并通过 `codex exec --image` 传给 Codex；普通文件、音频、视频会以本地路径写进 prompt。

## 致谢

本项目参考并改造自 [zarazhangrui/feishu-claude-code-bridge](https://github.com/zarazhangrui/feishu-claude-code-bridge)，保留 MIT 许可。
