...
const MAX_STDERR_LOG_LINES = 40;

const BRIDGE_SYSTEM_PROMPT = `# feishu-codex-code-bridge 运行约定

你正在 feishu-codex-code-bridge 里跑：把飞书/Lark 用户消息桥到本地 \`codex\` CLI。
...
授权流程要让 \`lark-cli\` 进程一直活到用户在浏览器里点完为止。bridge 在你的 run 结束之后会回收 codex，**你 spawn 的任何后台 bash 也会跟着死**——所以授权必须用"前台阻塞"的方式跑：
  ...
  run(opts: AgentRunOptions): AgentRun {
    ...
    child.stderr.on('data', (chunk: Buffer) => {
      ...
      while (nl !== -1) {
        ...
        if (line.trim()) {
          if (stderrLogLines < MAX_STDERR_LOG_LINES) {
            log.warn('agent', 'stderr', { line });
          } else if (stderrLogLines === MAX_STDERR_LOG_LINES) {
            log.warn('agent', 'stderr-omitted', { reason: 'too-many-lines' });
          }
          stderrLogLines++;
        }
    ...
    // Default 5s if caller didn't specify — codex often has live
...
function buildCodexArgs(opts: {
  ...
  const sandbox = opts.permissionMode === 'default' ? 'workspace-write' : opts.sandbox;
  const args = [
    '--ask-for-approval',
