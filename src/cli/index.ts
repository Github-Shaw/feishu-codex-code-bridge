...
program
  .command('migrate')
  .description(
    'Migrate from pre-0.1.11 setup: move ~/.config/feishu-codex-code-bridge/* and ' +
      '~/.cache/feishu-codex-code-bridge/* into ~/.feishu-codex-code-bridge/, and rewrite ' +
...
program
  .command('ps')
  .description('List running feishu-codex-code-bridge start processes (this machine)')
  .action(() => {
    runPs();
  });
...
const secrets = program
  .command('secrets')
  .description('Manage the bridge\'s encrypted secret keystore (~/.feishu-codex-code-bridge/secrets.enc)');

secrets
  .command('get')
  .description('Exec-provider protocol: read JSON request from stdin, write JSON response to stdout. Used by lark-cli-compatible config binding.')
  .action(async () => {
    await runSecretsGet();
  });
