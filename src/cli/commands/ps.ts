...
/**
 * Pretty-print the list of running feishu-codex-code-bridge processes.
 *
...
export async function runStopCli(target: string | undefined): Promise<void> {
  if (!target) {
    console.error('用法: feishu-codex-code-bridge stop <bot id 或序号>');
    process.exit(1);
  }
  const entry = resolveTarget(target);
  if (!entry) {
    console.error(`✗ 没找到匹配的 bot:${target}`);
    console.error('  用 `feishu-codex-code-bridge ps` 看可选目标。');
    process.exit(1);
  }
